                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-8">
                    <h3 className="text-md font-bold flex items-center gap-2">
                      <span
                        className={`w-3 h-3 rounded-full ${order.status === "Cancelled" ? "bg-red-500" : "bg-orange-500"}`}
                      ></span>
                      Status:{" "}
                      <span
                        className={
                  ...
                  {order.status !== "Cancelled" && (
                    <div className="flex items-center justify-between mb-8 px-2">
                      {steps.map((step, index) => (
                        ...
                        >
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center border-2 z-10 ${index <= currentIndex ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-slate-200 text-slate-400"}`}
                          >
                            {index <= currentIndex ? "✓" : index + 1}
                          </div>
                          <p
                            className={`text-[10px] font-bold mt-2 ${index <= currentIndex ? "text-slate-900" : "text-slate-400"}`}
                          >
                            {step}
                          </p>
                          {index < steps.length - 1 && (
                            <div
                              className={`absolute top-4 left-[50%] w-full h-0.5 ${index < currentIndex ? "bg-blue-600" : "bg-slate-200"}`}
                            ></div>
                          )}
                        </div>
                      ))}
